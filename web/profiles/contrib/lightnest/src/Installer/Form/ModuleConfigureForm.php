...
class ModuleConfigureForm extends ConfigFormBase {
  ...
  public function getFormId() {
    return 'lightnest_module_configure_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    $form['description'] = [
      '#type' => 'item',
      '#markup' => $this->t('Please select the components that you would like to install'),
    ];
    ...
    $module_file = Yaml::decode(file_get_contents( "{$profile->getPath()}/config/components.yml"));
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($form_state->getValues() as $key => $value) {
      if (strpos($key, 'install_modules') !== false && $value) {
        preg_match('/install_modules_(?P<name>\w+)/', $key, $values); // Matching names.
        $installModules[] = $values['name'];
      }
    }
    $buildInfo = $form_state->getBuildInfo();
    $install_state = $buildInfo['args'];
    $install_state[0]['lightnest_additional_modules'] = $installModules;
  ...
  protected function getEditableConfigNames() {
    // Not returning anything as configuration is not required.
    return [];
  }
}
