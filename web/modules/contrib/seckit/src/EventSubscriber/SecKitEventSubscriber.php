...
class SecKitEventSubscriber implements EventSubscriberInterface {
  ...
  public function seckitJsCssNoscript() {
    ...
    $body_open_match = preg_match('/<body.*>/', $content, $matches, PREG_OFFSET_CAPTURE);
    if ($body_open_match) {
      $length_of_body_tag = strlen($matches[0][0]);
      $body_open_position = $matches[0][1];
      $content = substr_replace($content, $this->seckitGetJsCssNoscriptMessage(), $length_of_body_tag + $body_open_position, 0);
      $this->response->setContent($content);
    }
  }
   ...
   */
  public function seckitGetJsCssNoscriptCode() {
...
EOT;
   ...
   *   Return the js and css code.
   */
  public function seckitGetJsCssNoscriptMessage($noscript_message = NULL) {
