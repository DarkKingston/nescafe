(function (Drupal, $) {
  Drupal.behaviors.lnNotificationPopupBlock = {
    attach: function (settings, context) {
      $('[data-notification-system-dispatch-webpushpopup]').once('initializieWebpushPopup').each(function () {
        ...
        Drupal.lnNotification.on('stateChange', newState => {
          switch (newState) {
            case 'incompatible':
              $block.addClass('hidden');
              break;
        ...
        const showBlockIfConditionsMatch = () => {
          ...
          if (Drupal.lnNotification.state === 'incompatible') {
          ...
          if (Drupal.lnNotification.state === null) {
