 ...
 *   label = @Translation("Content Hub Video Widget"),
 *   description = @Translation("Integrates with Content Hub service")
 * )
 */
class ContentHubVideoWidget extends WidgetBase implements ContainerFactoryPluginInterface {
  ...
  protected function buildSearch(&$form, FormStateInterface $form_state) {
    ...
    $search_results = $this->contentHubService->search($search_params['ln_contenthub_search_keyword'], ($search_params['ln_contenthub_pagesize'] + 1), $search_params['ln_contenthub_pageindex'], 'Video', $search_params['ln_contenthub_extra_param']);
  ...
  protected function prepareEntities(array $form, FormStateInterface $form_state) {
    ...
    if ($selected_videos = $form_state->get('ln_contenthub_selected', [])) {
      foreach ($selected_videos as $selected_video) {
        $video = json_decode($selected_video);

        $intellectual_property_rights = $this->contentHubService->checkIntellectualPropertyRights($video->id);
        ...
        try {
          ...
          $download_url = $video->previews->downloadUrl . '&apikey=' . $config->get('ln_contenthub_api_key');
          $image_name = str_replace(' ', '_', $video->name);
          ...
          $entity = $this->entityTypeManager->getStorage('media')->create([
            ...
            'name' => $video->name,
            'field_media_ln_contenthub_video' => [
              'target_id' => $file->id(),
            ],
            'field_media_ln_contenthub_id' => $video->id,
            'field_media_ln_contenthub_downl' => $video->previews->downloadUrl,
            'field_media_ln_conthub_thumbnail' => $video->previews->thumbnail,
            'field_media_ln_contenthub_viewex' => $video->previews->viewex,
            'field_media_ln_contenthub_mime_t' => $video->mimeType,
            'field_media_ln_contenthub_ipr' => $ipr,
            'field_media_ln_contenthub_ipr_ex' => $ipr_expiration_date,
            'field_media_ln_contenthub_bytes' => $video->bytes,
            'field_media_ln_content_last_mod' => $video->lastModified,
            'field_media_ln_contenthub_name' => $video->name,
            'field_media_ln_contenthub_path' => $video->path,
          ]);
    ...
    $form_state->set('ln_contenthub_prepared', $videos);
    return $videos;
  }
